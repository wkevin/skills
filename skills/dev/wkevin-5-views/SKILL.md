---
name: wkevin-5-views
description: 对代码仓库做有深度的架构分析。在传统五视图(逻辑/数据/开发/运行/物理)基础上,前置「上下文与心智模型」、后置「决策视图 + 行为视图(AI 项目专属) + 凝结 + 批判」,强制每个事实陈述都配 WHY 与判断。当用户提到"分析代码仓库"、"分析repo"、"分析项目架构"、"架构分析"、"代码架构"、"系统架构"、"五视图"、"为什么这么设计"、"设计哲学"、"架构取舍"、"harness"、"agent 项目分析"、"MCP 项目分析"且提供了仓库路径时触发此技能。
version: 2.0
---

# Skill: 五视图法++ - 代码仓库架构分析 (5-Views++ Method)

> **v2.0 重大重构说明**:v1.0 只产出"事实清点",在 AI/Agent 类项目上会**只见树木不见森林**(描述了 SQLite、MCP、FTS5,但没解释作者为什么这么选,也没看 prompt 层)。v2.0 加入**前置上下文视图、决策视图、行为视图(AI 专属)、凝结步骤、批判视角**,强制每个事实都配 WHY 和判断,把分析从"清点表"升级为"架构理解"。
