...
三件套有**严格的依赖方向**——这是本 skill 的**硬约束**,不是建议:
...
| ---------- | ----------------------------------------------------------------------- | -------------------------------------------------------- |
| `prd.md`   | `add.md §X` / `add.md DA-N` / `tasks.md UC-XX-NN` / `tasks.md IF-XX-NN` | `[add.md]` / `[tasks.md]` 链接 + 角色描述("X 是 Y 契约") |
| `add.md`   | `tasks.md UC-XX-NN` / `tasks.md IF-XX-NN`                               | 同上                                                     |
| `tasks.md` | (无下游,无约束)                                                         | 任意引用上游                                             |

**反例(违规)**:

- prd.md 写 "详见 add.md §DA-12" → 引用了下游 ADR ❌
- add.md 写 "UC-01-01 添加 repo 流程" → 引用了下游 UC ❌
- prd.md 写 "tasks.md = UC/IF 全量定义 + Sprint 计划 + 状态 + commit 历史" → 展开下游文档内容 ❌

**正例(合规)**:

- prd.md 写 "详见架构文档(单用户本地决策)" → file-level 指引 ✅
- add.md 写 "AI 建议在 tag-editor 内以虚线 border + Sparkles 标记" → 描述语义,无下游 ID ✅
- prd.md 写 "见 [tasks.md](./tasks.md)" → file-level 链接 ✅

**WHY**:

- 下游文档会被频繁迭代(tasks.md 加新 UC);上游文档若硬编码下游编号,下游一变上游就断链
...
```bash
# prd.md → 下游引用 (应为 0)
grep -nE 'add\.md §|add\.md#|tasks\.md (UC|IF)-[0-9]+-[0-9]+' docs/prd.md
...
| 5   | **"文档定位" 集中放 README**             | `docs/README.md` 加"## 文档定位"段,讲 prd/add/tasks 关系 + 依赖方向,主文档不重复                       |
...
- `docs/tasks.md`(catalog + 状态 + Backlog)
...
| §6/§7 里程碑缺失    | 无                        | 新 §6 roadmap 或 §7 里程碑 |                     | +15 行(可选) |
